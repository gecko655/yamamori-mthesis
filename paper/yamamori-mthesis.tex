...
Zimmermannらは、改版履歴を解析することにより、変更される可能性のあるメソッド、フィールド等のコード要素を推薦する変更推薦ツール、eROSE\cite{Zimmermann:2005}を提案し、改版履歴を用いることで高い推薦精度で変更推薦が行えることを示した。
...
Mylyn\cite{Kersten:2005}は公開されている操作履歴記録ツールであり、実開発で記録された7年分の操作履歴が利用可能である。
しかし、Mylynで記録された操作履歴は他の細粒度操作履歴と異なり、ファイルの変更を伴う操作であったかどうかや操作の時系列の情報を復元することが出来ないため、既存の操作履歴を利用した変更支援手法をそのまま適用することが出来ない。
...
しかし、前節で述べたように、Mylynの操作履歴は既存の操作履歴を用いる変更支援手法を適用することができない。
...
Mylynログに記録される操作の情報は、Mylynのタスク毎に操作された成果物が記録されているため、Gitの改版履歴に含まれる変更がコミット毎に記録されることと対応している。
...
本章では、Mylynログに細粒度な情報を付け足すことなく、これまでに記録されたMylynログを用いて変更支援を行なえるかどうかを調査するため、既存のMylynログを適用できる新たな変更支援手法を提案する。
...
すなわち、$r_j$の順位は$recom_{Rule}$に含まれる推薦結果のうち$score_j$と同じかより高いスコアの成果物の数（$score_j$を含む）である。
