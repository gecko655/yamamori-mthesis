...
トランザクション集合$T$内で見つかったある相関ルール$X \Ra Y$において
$support$とは全体のトランザクション数のうち$X$および$Y$を含んでいるトランザクションの割合であり、
$\confidence$とは$X$を含むトランザクション数のうち$X$および$Y$を含んでいるトランザクションの割合である。
これらを形式的に表すと以下のようになる。
\begin{eqnarray}
  support_{T,X \Ra Y} &=& \frac{| X \cup Y |}{|T|}\\
  \confidence_{T,X \Ra Y} &=& \frac{| X \cup Y |}{|X|}
\end{eqnarray}
なお、$minsup$については、\cite{Zimmermann:2005}のように全てのトランザクション数に対する割合ではなく、$X$および$Y$を含むトランザクション数を利用している研究もある。\cite{Zimmermann:2005}ではこのような数値を{\it minimum support count}と呼んでいる。
