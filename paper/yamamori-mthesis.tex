...
\begin{comment}
co-refの相関ルールを$Rule_{ref} =$ \\$\{(l_1,r_1), (l_2,r_2), \dots, (l_m,r_m)\}$とする。
ここで($l_i$,$r_i$)は相関ルール$L \Ra R$を表し、$l_i$は左辺の成果物集合、$r_i$は右辺の成果物を表す（\ref{extractrules_sec}節参照）。
変更推薦を行なう対象の参照履歴を$Reference = \{ref_1, ref_2, \dots, ref_n\}$、変更履歴を$Change = \{chg_1, chg_2, \dots, chg_n\}$とする。
ここで$ref_i$と$chg_j$はそれぞれ参照セット、変更セットであり、$i=j$ならばこれらのセットは同じ開発行動を表す参照セット、変更セットである。
すべての変更された成果物$c \in chg_i$に対して、 変更推薦$recom_{i,c}$
\end{comment}
