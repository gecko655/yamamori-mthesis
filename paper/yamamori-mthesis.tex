...
\begin{figure}[tb]
  ...
  \label{scoring_plot}
  \caption{各評価値でランキングを行なった時のスコア付けの概略}
\end{figure}
...
$getScore(rule_j)$（\ref{ranking_sec}節参照）は、
\ref{rulesize_sec}節および\ref{mainresult_sec}節では$\confidence$を用い、
\ref{varyscoring_sec}節では$getScore(rule_j)$を変化させて実験を行なう。
