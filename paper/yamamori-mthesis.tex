...
本章では、\ref{experiment_chap}章で説明した実験の結果について述べる。

\section{実験設定}
LCExtractorに使われているA-prioriアルゴリズムはminimum support（最小支持度,$minsup$）とminimum confidence（最小信頼度,$\minconf$）の2つのパラメータが必要となる。
\section{相関ルールの数}
\section{$Precision$と$Recall$}
\section{$MRR$と$Recall$}
\section{$\fmeasure$}
