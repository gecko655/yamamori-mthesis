...
\section{本論文のアプローチ}\label{approach_sec}
...
この手法は改版履歴を用いる既存研究で提案された、共変更の相関ルールを用いる手法をベースとし、操作履歴の特徴を考慮したランキング手法を用いる。
...
特に、改版履歴を用いた変更支援手法は変更推薦を行える回数が少ないことが問題だったが、操作履歴を用いることで推薦回数が増やせることや、推薦可能な回数を同数に保った時に推薦精度が向上することが示せた。
これは、操作履歴を用いたことで、開発者が変更を行なうことがまれな成果物に対しても相関ルールを抽出することが出来たためと考えられる。
また、変更推薦結果のランキングに相関ルールの確信度を用いるだけでなく、データセット内での出現頻度を用いたり、これらを両方考慮することで、推薦可能な回数を維持しながら、変更推薦の精度を改善することができた。
...
\begin{table}[t]
  \begin{center}
    ...
    \begin{tabular}{l|lll|lll}
      ...
      \hline
      $Recall \geq 0.1$のときの &{\bf 0.196}& 0.143 & (0.006, 0.3) &0.158&0.146 & (0.002, 0.3) \\
      最大$MRR$&&&&&\\
      \hline
    \end{tabular}
  \end{center}
...
現在、本拡張をMylynプロジェクトのコミュニティに提出し、変更を適用してもらえるよう交渉しているところである。
今後、この拡張を一般の開発者に導入してもらい、操作履歴を記録、回収する作業を行なう必要がある。
