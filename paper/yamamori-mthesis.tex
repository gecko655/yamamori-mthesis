...
\section{目的}
改版履歴を利用した変更推薦手法の効果はすでに実証されており\cite{Zimmermann:2005}、この手法と同様に、変更された成果物以外の情報も記録されている操作履歴を利用しても変更推薦手法ができるのではないかと考えられる。
しかし、この検証を行なった研究は存在するが\cite{6233415,KatoJapanese:2011,ss2012-76,ss2013-84,Yamamori:2016}、検証時に使ったデータセットが小さく、またデータセットを収集することが非常に困難なため、詳細かつ十分な検証を行なうことが難しいという問題がある。
...
\begin{figure}[tb]
  ...
  \subfigure[$\confidence*support$をもとにスコア付けしたとき]{
...
また、\ref{ranking_sec}節で述べたように$\getScore(rule_j)$は$\confidence$、$suport$、$\confidence*support$の3通りがある。
...
\ref{varyscoring_sec}節では、
\ref{rulesize_sec}節および\ref{mainresult_sec}節の結果を考慮し、
変更推薦を行える回数を維持しながら推薦精度を向上することを目的として、
$\getScore(rule_j)$を変更することによって変更推薦の精度がどのように変化するかを検証する。
$minsup$は0.002で固定として、
$\getScore(rule_j)$は$\confidence$、$suport$、$\confidence*support$の3通りを用いた。
$\minconf$は0.1から0.9まで0.1刻みで9種類を用いた。
また、\ref{varyscoring_sec}節で後述する理由によりファイルレベルにおいてのみ実験を行う。
...
\end{figure}
$\getScore(rule_j)$（\ref{ranking_sec}節参照）を変更した時の$\fmeasure$を図\ref{f_fmeasure_rank}に、$MRR$と$Recall$の関係を図\ref{f_mrgraph_rank}に、それぞれ示す。
